...
func New(fileName string) Configuration {
	...
	cfg.rootDirPath = os.Getenv("TX_HOME")
	if cfg.rootDirPath == "" {
...
func (cfg Configuration) InitDatabase(name string) (*gorm.DB, error) {
	...
	for _, database := range cfg.Databases {
		if strings.EqualFold(database.Name, name) {
			...
			case "mysql":
				connectionString = fmt.Sprintf("%s:%s@tcp(%s)/%s?charset=utf8&parseTime=true&multiStatements=true&timeout=30s&autocommit=1", database.Username, database.Password, database.Address, database.DBName)
			}
